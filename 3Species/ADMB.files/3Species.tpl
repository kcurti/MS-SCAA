//AllTogether, Attempt 130
     //Adding parameter bounds to aFt
//Function Details:
//  data_section = MSP Version, 09-22-10
//  parameter_section = MSP Version, 10-21-10
//  procedure to top_of_main sections = MSP Version, 10-09-10
//  calc_initial_states = MSP Version, 09-22-10
//  calc_suitability = 02-08-10 Version (works with new Rsim input files)
//  calc_predation_mortality = 09-21-10, Other Food penalty
...
//  objective_function through final section = MSP Version, 09-30-10 (OFood pen Diagnostics)
//  report_section = MSP Version, 09-22-10
...
//Debug statements
  ...
  //20's = Suitability function
      ...
      //  = 23:  Outputs size pref, suitability and scaled suitability for one pred, prey combo and then exits
...
DATA_SECTION
  ...
  init_int aAge1ph;                //aAge1 estimation phase
  init_int aFtph;                     //aFt estimation phase
  init_int dAge1ph;                //dAge1 estimation phase
  init_int dFtph;                     //dFt estimation phase
  init_int ficph;                      //Survey selectivity, FICsel, estimation phase
  init_int fishph;                    //Fishery selectivity, agesel, estimation phase
  init_int Yr1ph;                    //Yr1 estimation phase
  ...
  ivector rhosp(1,nsp);          //The number of estimable rhos for each species (rhosp>0 if species consumes other modeled species)
    ...
    //If other food is not included in the model, uncomment the following: //for (i = 1; i<=nsp; i++) {if (rhosp(i)>0) {rhosp(i)--;} }
    !!nrho = sum(rhosp);       //Total number of estimable rho parameters
  init_ivector Etaph(1,nint);  //Phase in which each eta is estimated
  init_ivector Sigph(1,nint);  //Phase in which each sigma is estimated
  ...
  init_vector aEta(1,nint);     //Average value for Eta (preferred predator/prey weight ratio, log space)
  init_vector sEta(1,nint);     //Standard deviation in Eta (preferred predator/prey weight ratio, log space)
  init_vector aSig(1,nint);     //Average value for Sigma (variance in predator/prey weight ratio, log space)
  init_vector sSig(1,nint);     //Standard deviation in Sigma (variance in  predator/prey weight ratio, log space)
  ...
  init_vector Ewt(1,nint);              //Weight for Eta penalty term
  init_vector Swt(1,nint);              //Weight for Sigma penalty term
  ...
  init_matrix CB(1,nsp,1,nage);               //Age-specific consumption:biomass ratios
  ...
  init_int eof;
    ...
    //The first phase where trophic calculations are conducted; **Assumes Etaph/Sigph are vectors
    dvector phtmp(1,(nrho+2*nint));
      phtmp(1,nrho) = Rhoph;
      phtmp((nrho+1),(nrho+nint)) = Etaph.shift(nrho+1);
      phtmp((nrho+nint+1),(nrho+2*nint)) = Sigph.shift(nrho+nint+1);
    tphase = posmin(phtmp);
      //Shifting the indices of etaph and sigph back to (1,nint)
      Etaph.shift(1);  Sigph.shift(1);

    //The first and last years of each FH bin:
    binfyr.fill_seqadd(FHfyr,binsize);
    binlyr = binfyr + binsize-1;
    binlyr(nbins) = FHlyr;  //Note that the last FHbin is only comprised of two years; will eventually likely need to fix this issue
    ...
    //Calculating predator:prey weight ratios for years with FH data
    for (pd=1; pd<=nsp; pd++)
      {
...
PARAMETER_SECTION
  ...
  vector Epen(1,nint);      //Penalty function for Eta, the preferred predator/prey weight ratio
  vector tEpen(1,nint);     //Total Eta penalty for each species
  vector Spen(1,nint);      //Penalty function for Sigma, the variance in Eta
  vector tSpen(1,nint);     //Total Sigma penalty for each species
  ...
  init_bounded_number_vector iEta(1,nint,-20,20,Etaph);      //Preferred predator/prey weight ratio, log space
  init_bounded_number_vector iSig(1,nint,-15,15,Sigph);       //Variance in predator/prey weight ratio, log space
  init_bounded_number_vector iRho(1,nrho,-30,100,Rhoph); //General prey species preference coefficient

  init_vector aAge1(1,nsp,aAge1ph);                        //Average annual recruits, log space
  init_bounded_vector aFt(1,nsp,-20,10,aFtph);          //Average annual fishing mortality rates; averaged over yrs where TotC > 0, log space
  init_bounded_matrix dAge1(1,nsp,fyr+1,lyr,-15,5,dAge1ph); //Annual deviations in recruits, log space
  init_bounded_matrix dFt(1,nsp,1,nFt,-5,5,dFtph);                 //Annual deviations in species-specific Fs in yrs where TotC > 0, log space

  matrix Ft(1,nsp,fyr,lyr);                                                     //Annual species-specific fishing mortality rates
  init_bounded_matrix FICsel(1,nsp,1,FICnsel,0,1,ficph)        //Age-specific survey selectivity parameters
  init_bounded_matrix agesel(1,nsp,1,nsel,0,1,fishph);            //Age-specific fishery selectivity/vulnerability parameters

  init_matrix Yr1(1,nsp,1,nage,Yr1ph);                      //Year 1 age-specific N's, log space
  ...
  matrix Eta(1,nsp,1,nsp);
  matrix Sig(1,nsp,1,nsp);
...
PROCEDURE_SECTION
  ...
  if (debug == 23)  {
    ...
    cout<<"coutyr\n"<<coutyr<<endl<<"pdtmp\n"<<pdtmp<<endl<<"pytmp\n"<<pytmp<<endl
          <<"Eta\n"<<Eta(pdtmp,pytmp)<<endl<<"Sig\n"<<Sig(pdtmp,pytmp)<<endl<<"Rho\n"<<Rho(pdtmp,pytmp)<<endl
...
FUNCTION calc_initial_states
  ...
  */
  ...
  Eta.initialize();  Sig = -9999.999;  Rho.initialize();  //Sig set to -9999 to prevent dividing by zero when species do not interact
  N.initialize();  B.initialize();  
  Bpen.initialize();  Ypen.initialize();  Rpen.initialize();  Epen.initialize();  Spen.initialize();  Open.initialize();
  ...
  if (trophic)  {sumCon.initialize();  G.initialize();  Nu.initialize();  Snu.initialize();  Avail.initialize();  scAv.initialize();
  ...
  for (j = 1; j<=nint; j++)
    ...
    pd = pred(j);
    py = prey(j);
    Eta(pd,py) = mfexp(iEta(j));
    Sig(pd,py) = mfexp(iSig(j));
  ...
  if (debug == 10) {cout<<"Eta\n"<<Eta<<endl<<"Sigma\n"<<Sig<<endl<<"Rho\n"<<Rho<<endl<<"pred\n"<<pred<<endl<<"prey\n"
                                   <<prey<<endl<<"pdct\n"<<pdct<<endl<<"rhoct\n"<<rhoct<<endl<<"rhosp\n"<<rhosp<<endl; exit(-1);}
  if (debug == 11) {cout<<"Eta\n"<<Eta<<endl<<"Sigma\n"<<Sig<<endl<<"Rho\n"<<Rho<<endl;}
...
FUNCTION calc_suitability
  /*
  Function calculates 1) predator size-preference, G, 2) prey suitability, Nu, and 3) scaled suitability matrix 
    across prey species, Snu
  */
  for (pd=1; pd<=nsp; pd++)
    {
    for (py=1; py<=nsp; py++)
      ...
      if (Sig(pd,py)!=-9999.999)
        {
         G(pd)(t).sub(fage(py),lage(py)) = exp(-square( log(WtRatio(pd)(t).sub(fage(py),lage(py))) - Eta(pd,py) )/(2. *square( Sig(pd,py) )));
  ...
  */
  ...
  for (pd=1; pd<=nsp; pd++)
    ...
    for (pdA=1; pdA<=nage(pd); pdA++)
      ...
      Consum(pd,t,pdA) = CB(pd,pdA) * B(pd,t,pdA) * scAv(pd,t,pdA); //Simulation eq. used Ing and N; GB equation uses CB and B
      }  //end of predator age loop
...
FUNCTION calc_negative_loglikelihood
  ...
  */
  ...
  tEpen.initialize();                                           //Total Eta penalty for each species
  tSpen.initialize();                                           //Total Sigma penalty for each species
  ...
  //Penalty terms
  Epen = square(elem_div((iEta - aEta),sEta));  //Likelihood penalty corresponding to the prior on Eta
  Spen = square(elem_div((iSig  - aSig),sSig));  //Likelihood penalty corresponding to the prior on Sigma
  ...
  tEpen = elem_prod(Ewt,Epen);   //Total Eta penalty for each species
  tSpen = elem_prod(Swt,Spen);   //Total Sigma penalty for each species
  ...
  ofv = sum(ofvsp) + sum(tBpen) + sum(tYpen) + sum(tRpen) + sum(tEpen) + sum(tSpen) + tOpen + sum(Devs);
  ofv_ideal = sum(ofvsp_ideal) + sum(tBpen) + sum(tYpen) + sum(tRpen) + sum(tEpen) + sum(tSpen) + tOpen + sum(Devs);
  ...
  if (debug == 66 | debug == 3) {
    ...
    cout<<"tEpen: "<<tEpen<<"  tSpen: "<<tSpen<<endl;
